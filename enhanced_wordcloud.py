    ...
    if connections and len(connections) > 0:  # Only process if we have valid connections
        ...
        for i, conn in enumerate(connections):
            try:
                if len(conn) != 3:
                    print(
                        f"Warning: Invalid connection format at index {i}: {conn}. Expected (source, target, weight).")
                    continue
...
def main():
    ...
    if args.bp_scores:
        if os.path.exists(args.bp_scores):
            ...
            # Create nodes dictionary with the full data structure
            nodes = {}
            for bp, data in bp_data.items():
                if isinstance(data, dict):
                    nodes[bp] = data
                else:
                    # Handle case where data is just a score
