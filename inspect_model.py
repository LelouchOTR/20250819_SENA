            ...
            for key, tensor in model.state_dict().items():
                if hasattr(tensor, 'shape'):
                    ...
                    if len(tensor.shape) == 2 and tensor.shape[0] == tensor.shape[1]:
                        ...
                        print(
                            f"  - Is symmetric: {torch.allclose(tensor, tensor.T) if tensor.shape[0] == tensor.shape[1] else 'N/A'}")
                else:
        ...
        # For non-tuple models, try to find state_dict
            ...
            for key, tensor in model.state_dict().items():
                ...
                if hasattr(item, 'causal_graph'):
                    ...
                    print(
                        f"causal_graph shape: {item.causal_graph.shape if hasattr(item.causal_graph, 'shape') else 'N/A'}")
